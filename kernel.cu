...
bool isPositiveInteger(std::string const &s) {
  return !s.empty() &&
...
*/
Monomial normalisedMonomialLeastCommonMultiple(Monomial const &monomial1,
  Monomial const &monomial2) {
...
*/
Monomial dividedByNormalisedMonomial(Monomial const &divisible,
  Monomial const &divisor) {
...
Polynomial multiplyByNormalisedMonomial(Polynomial const &polynomial,
...
Polynomial addPolynomials(Polynomial const &polynomial1,
  Polynomial const &polynomial2, int prime) {
...
Polynomial generateRandomSortedPolynomial(size_t nVariables,
  int maxVariableDegree, int prime,
  size_t maxNMonomials) {
...
void subtract(Polynomial &polynomial1, Polynomial const &polynomial2,
  int prime) {
...
Polynomial getReducedPolynomial(Polynomial const &polynomial,
  ...
  auto current = polynomial;
  for (size_t basisPolynomialI = 0; basisPolynomialI < basis.polynomials.size();
    ++basisPolynomialI) {
    auto basisPolynomial = basis.polynomials[basisPolynomialI];
    auto major = getMajorMonomial(basisPolynomial);
    for (size_t monomialI = 0; monomialI < current.monomials.size();
      ++monomialI) {
...
Polynomial getPolynomialWithEliminatedMajorMonomials(
  ...
  auto multipliedPolynomial1 =
    multipliedByMonomial(polynomial1, multiplier1, prime);
  auto multipliedPolynomial2 =
    multipliedByMonomial(polynomial2, multiplier2, prime);
...
PolynomialBasis generateRandomPolynomialBasis(size_t nVariables,
  int maxVariableDegree, int prime,
  size_t maxNMonomials,
  size_t nPolynomials) {
...
/*
    Возвращает случайный ненулевой многочлен из идеала, создаваемого базисом
   basis.
*/
Polynomial generateRandomPolynomialFromIdeal(PolynomialBasis const &basis,
  int prime) {
  Polynomial current;
  for (auto const &polynomial : basis.polynomials) {
    auto randomCoefficient = rand() % prime;
    auto multiplied =
      multipliedByCoefficient(polynomial, randomCoefficient, prime);
    subtract(current, multiplied, prime);
  }
  return isZero(current) ? generateRandomPolynomialFromIdeal(basis, prime)
    : current;
}
...
void testGetPolynomialWithEliminatedMajorMonomials() {
  auto result1 =
    getPolynomialWithEliminatedMajorMonomials({ "1", 1 }, { "1", 1 }, 7);
  assert(result1 == Polynomial());

  auto result2 =
    getPolynomialWithEliminatedMajorMonomials({ "a^1", 2 }, { "b^1", 2 }, 7);
  assert(result2 == Polynomial());

  auto result3 = getPolynomialWithEliminatedMajorMonomials({ "a^1 + b^1", 2 },
    { "a^1", 2 }, 7);
  assert(result3 == Polynomial("b^1", 2));

  auto result4 = getPolynomialWithEliminatedMajorMonomials({ "a^1", 2 },
    { "a^1 + b^1", 2 }, 7);
  assert(result4 == Polynomial("6*b^1", 2));
}

void testGetGrobnerBasis(size_t nVariables, size_t nPolynomials,
